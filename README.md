...
Procedure call to long-running multi-threaded services within the same process.
It is slightly inspired by ROS' Service but instead of being multiple processes,
all services are contained within the same process.
...
/// together and return the total for demonstration purposes.
struct AdderService {
    s: Sender<(
...
fn main() {
    ...
    thread::spawn(move || {
        for _ in 0..1000 {
            let sender = sender_0.clone();
            let request = AdderServiceRequest::Add(5, 2);
            let response = AdderService::call(sender, request);
            match response {
